...
def plot_sensor_map(value_column: str = "P1", title: str = "Sensor Map") -> str:
    """
    Create an interactive map visualization of sensor locations using Plotly.
    
    Args:
        value_column (str): Column to use for marker color/size (e.g., 'P1', 'P2')
        title (str): Plot title
    ...
    """
    if st.session_state.dataset is None:
        return "Error: No dataset loaded. Please load a dataset first using fetch_latest_sensor_data."
    ...
    try:
        ...
        if 'lat' not in df.columns or 'lon' not in df.columns:
            return "Error: Dataset must contain 'lat' and 'lon' columns for map visualization."
        
        # Clean and validate the value column
        value_column = clean_column_name(value_column)
        
        # Find matching column if not exact
        if value_column not in df.columns:
            value_column = find_matching_column(df, value_column)
            if not value_column:
                # Default to first numeric column that's not lat/lon
                numeric_cols = [col for col in df.select_dtypes(include=['number']).columns 
                               if col not in ['lat', 'lon']]
                if numeric_cols:
                    value_column = numeric_cols[0]
                    st.info(f"No matching column found. Using '{value_column}' for values.")
                else:
                    return "Error: No suitable numeric column found for map values."
        
        # Create a copy of the dataframe with only the necessary columns
        map_df = df[['lat', 'lon', value_column]].copy()
        
        # Drop rows with missing lat, lon or value
        map_df = map_df.dropna(subset=['lat', 'lon', value_column])
        
        if len(map_df) == 0:
            return "Error: No valid data points with latitude, longitude, and values."
        ...
        # Generate insights
        avg_value = map_df[value_column].mean()
        max_value = map_df[value_column].max()
        max_location = map_df.loc[map_df[value_column].idxmax()]
        
        insights = f"""
        ## Map Insights
        
        **Total Sensors:** {len(map_df)} sensors displayed on the map
        **Average {value_column}:** {avg_value:.2f}
        **Maximum {value_column}:** {max_value:.2f} at location [Lat: {max_location['lat']:.4f}, Lon: {max_location['lon']:.4f}]
        """
...
def plot_time_series(columns: Union[List[str], str] = "P1,P2", 
                    title: str = "Sensor Readings Over Time",
                    rolling_window: int = 0,
                    show_trend: bool = False) -> str:
    """
    Create an interactive time series visualization using Plotly.
    
    Args:
        columns (Union[List[str], str]): List of columns or comma-separated string of columns to plot
        title (str): Plot title
        rolling_window (int): Window size for rolling average (0 to disable)
        show_trend (bool): Whether to show trend line
    ...
    try:
        ...
        timestamp_cols = [col for col in df.columns if any(time_word in col.lower() 
                         for time_word in ['time', 'date', 'timestamp'])]
        
        if not timestamp_cols:
            return "Error: Dataset must contain a timestamp column for time series visualization."
        ...
        if not pd.api.types.is_datetime64_any_dtype(df[time_col]):
            try:
                ...
                return f"Error converting {time_col} to datetime: {str(e)}"
        ...
        for col in columns:
            if col in df.columns:
                ...
                matched_col = find_matching_column(df, col)
                if matched_col:
                    valid_columns.append(matched_col)
                    st.info(f"Using '{matched_col}' for requested column '{col}'")
        
        if not valid_columns:
            # Default to first numeric column
            numeric_cols = df.select_dtypes(include=['number']).columns.tolist()
            if numeric_cols:
                valid_columns = [numeric_cols[0]]
                st.info(f"No valid columns specified. Using '{numeric_cols[0]}'")
            else:
                return "Error: No suitable numeric columns found for time series."
        
        # Create a copy with only needed columns
        plot_df = df[[time_col] + valid_columns].copy()
        ...
        for col in valid_columns:
            ...
            if rolling_window > 0:
                rolling_data = plot_df[col].rolling(window=rolling_window).mean()
                fig.add_trace(go.Scatter(
                    x=plot_df[time_col],
                    y=rolling_data,
                    mode='lines',
                    line=dict(width=3, dash='dash'),
                    name=f"{col} ({rolling_window}-point MA)",
                    hovertemplate=f"%{x}<br>{rolling_window}-point MA: %{{y:.2f}}<extra></extra>"
                ))
            ...
            if show_trend:
                ...
                mask = ~np.isnan(plot_df[col])
                if sum(mask) > 1:  # Need at least 2 points for linear regression
                    z = np.polyfit(x_numeric[mask], plot_df[col][mask], 1)
                    ...
                    fig.add_trace(go.Scatter(
                        ...
                        name=f"{col} trend",
                        hovertemplate=f"%{x}<br>Trend: %{{y:.2f}}<extra></extra>"
                    ))
        ...
        # Generate insights
        insights = "## Time Series Insights\n\n"
        
        for col in valid_columns:
            # Calculate statistics
            latest_value = plot_df[col].iloc[-1] if not plot_df.empty else None
            avg_value = plot_df[col].mean()
            max_value = plot_df[col].max()
            min_value = plot_df[col].min()
            
            # Calculate trend direction
            if len(plot_df) > 1:
                first_valid = plot_df[col].iloc[0] if not pd.isna(plot_df[col].iloc[0]) else plot_df[col].dropna().iloc[0] if not plot_df[col].dropna().empty else None
                last_valid = plot_df[col].iloc[-1] if not pd.isna(plot_df[col].iloc[-1]) else plot_df[col].dropna().iloc[-1] if not plot_df[col].dropna().empty else None
                
                if first_valid is not None and last_valid is not None:
                    change = last_valid - first_valid
                    percent_change = (change / first_valid) * 100 if first_valid != 0 else float('inf')
                    
                    trend_direction = "increasing" if change > 0 else "decreasing" if change < 0 else "stable"
                    insights += f"**{col}**: Currently at {latest_value:.2f}, {trend_direction} trend ({percent_change:.1f}% change)\n"
                    insights += f"- Range: {min_value:.2f} to {max_value:.2f}, Average: {avg_value:.2f}\n\n"
        ...
        st.error(traceback.format_exc())
        return f"Error creating time series visualization: {str(e)}"


def create_sensor_dashboard(value_columns: Union[List[str], str] = "P1,P2",
                          title: str = "Sensor Data Dashboard") -> str:
    """
    Create an interactive dashboard with map and time series visualizations using Plotly.
    
    Args:
        value_columns (Union[List[str], str]): List of columns or comma-separated string of columns to visualize
    ...
    try:
        ...
        if isinstance(value_columns, str):
            # Handle JSON input
            if '{' in value_columns and '}' in value_columns:
                params = parse_json_input(value_columns)
                if 'columns' in params:
                    value_columns = params['columns']
                    
            # Handle comma-separated list
            if isinstance(value_columns, str) and ',' in value_columns:
                value_columns = [col.strip() for col in value_columns.split(',')]
            elif isinstance(value_columns, str):
                value_columns = [value_columns.strip()]
        
        # Clean column names
        value_columns = [clean_column_name(col) for col in value_columns]
        ...
        if not valid_columns:
            ...
            if numeric_cols:
                ...
                return "Error: No suitable numeric columns found for visualization."
        
        # Display dashboard title
        st.title(title)
        
        # Create tabs for different visualizations
        map_tab, time_tab, stats_tab = st.tabs(["Map View", "Time Series", "Statistics"])
        
        # Create a column selector in the sidebar
        with st.sidebar:
            st.subheader("Dashboard Controls")
            
            # Select column for map visualization
            if len(valid_columns) > 1:
                ...
                selected_map_col = valid_columns[0]
            ...
            if show_rolling_avg:
                window_size = st.slider("Rolling window size", min_value=2, max_value=20, value=5)
            else:
                window_size = 0
        ...
        with map_tab:
            if 'lat' in df.columns and 'lon' in df.columns:
                st.subheader(f"Sensor Locations - {selected_map_col} Values")
                
                # Create map visualization
                map_df = df[['lat', 'lon', selected_map_col]].copy().dropna()
                ...
                if len(map_df) > 0:
                    fig_map = px.scatter_mapbox(
                        ...
                        size=selected_map_col,
                        color_continuous_scale="Viridis",
                        size_max=15,
                        zoom=3,
                        hover_data={selected_map_col: True}
                    )
                    ...
                    with col1:
                        st.metric("Total Sensors", f"{len(map_df)}")
                    with col2:
                        st.metric("Average Value", f"{map_df[selected_map_col].mean():.2f}")
                    with col3:
                        st.metric("Maximum Value", f"{map_df[selected_map_col].max():.2f}")
                else:
                    st.warning("No valid location data available for map visualization.")
            else:
                st.warning("Location data (lat, lon) not found in the dataset.")
        ...
        with time_tab:
            ...
            if timestamp_cols:
                ...
                for col in valid_columns:
                    ...
                    # Add rolling average if requested
                    if show_rolling_avg:
                        rolling_data = plot_df[col].rolling(window=window_size).mean()
                        fig_time.add_trace(go.Scatter(
                            x=plot_df[time_col],
                            y=rolling_data,
                            mode='lines',
                            line=dict(width=3, dash='dash'),
                            name=f"{col} ({window_size}-point MA)",
                            hovertemplate=f"%{x}<br>{window_size}-point MA: %{{y:.2f}}<extra></extra>"
                        ))
                ...
                # Create metrics for each column
                metrics_cols = st.columns(len(valid_columns))
                
                for i, col in enumerate(valid_columns):
                    # Calculate trend direction
                    if len(plot_df) > 1:
                        first_valid = plot_df[col].dropna().iloc[0] if not plot_df[col].dropna().empty else None
                        last_valid = plot_df[col].dropna().iloc[-1] if not plot_df[col].dropna().empty else None
                ...
                st.warning("No timestamp column found in the dataset.")
